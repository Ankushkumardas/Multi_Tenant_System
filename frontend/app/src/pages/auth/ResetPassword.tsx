...
const ResetPassword = () => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        if (!password || !token) {
            setStatus("error");
            setMessage(!token ? "Missing reset token in URL" : "Password is required");
            return;
        }
        setStatus("loading");
        try {
                ...
                >
                    ...
                    {status === "success" ? (
                        ...
                        <form className="space-y-5" onSubmit={handleSubmit}>
                            ...
                            <button type="submit" disabled={status === "loading" || !token} className="w-full h-12 flex justify-center items-center rounded-xl bg-gray-900 text-white text-[14px] font-semibold hover:bg-gray-800 transition-all active:scale-[0.98] shadow-md shadow-gray-900/20 disabled:opacity-70">
                                {status === "loading" ? "Processing..." : "Save Password"}
                            </button>
                        </form>
                    )}
                </motion.div>
